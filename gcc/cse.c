...
static void remove_from_table(struct table_elt *, unsigned, const char *);
...
static char * reconcatenate(const char * s1, const char * s2, char * s3);
static void invalidate(rtx, enum machine_mode, const char *);
...
static void
remove_from_table(elt, hash, name)
...
const char *name;
{
    ...
    if (static_file) {
        fprintf(static_file, "(Removed hash value %d from %s)\n  ", hash, name);
        print_rtl(static_file, elt->exp);
        fprintf(static_file, "\n");
    }
...
enum machine_mode mode;
{
    ...
    if (static_file) {
        fprintf(static_file, "(Added hash value %d, mode %s)\n  ", hash, GET_MODE_NAME(mode));
        print_rtl(static_file, x);
        fprintf(static_file, "\n");
    }
...
struct table_elt *class1, *class2;
{
    ...
    for (elt = class2; elt; elt = next) {
        ...
        if (GET_CODE(exp) == REG || exp_equiv_p(exp, exp, 1, 0)) {
            ...
            remove_from_table(elt, hash, "merge_equiv_classes");
...
/* Returns the concatenation of two strings in a malloc'd pointer.
   Use free() once the string is no longer needed */
static char * concatenate(const char * s1, const char * s2)
{
...
/* Like concatenate except frees the third pointer given (In the case of assigning to one of the concatenated strings) */
static char * reconcatenate(const char * s1, const char * s2, char * s3)
{
    const size_t len1 = strlen(s1);
    const size_t len2 = strlen(s2);
...
static void
invalidate(x, full_mode, calling_fn_name)
rtx x;
...
const char *calling_fn_name;
{
    ...
    // TODO: line numbers
    if (static_file) {
        fn_names = concatenate("invalidate from ", calling_fn_name);
        if (fn_names == NULL) {
    ...
    if (GET_CODE(x) == REG) {
        ...
        if (regno >= FIRST_PSEUDO_REGISTER) {
            ...
            while ((elt = lookup_for_remove(x, hash, GET_MODE(x)))) {
                remove_from_table(elt, hash, fn_names);
            }
        } else {
                ...
                for (hash = 0; hash < NBUCKETS; hash++)
                    for (p = table[hash]; p; p = next) {
                        ...
                        if (tendregno > regno && tregno < endregno) {
                            remove_from_table(p, hash, fn_names);
                        }
                    }
        }
        free(fn_names);
    ...
    if (static_file) {
        fn_names = reconcatenate("invalidate from ", fn_names, fn_names);
        if (fn_names == NULL) {
            fprintf(stderr, "Could not allocate for concatenate at %s:%d.\n", __FILE__, __LINE__);
            abort();
        }
    ...
    if (GET_CODE(x) == SUBREG) {
        ...
        invalidate(SUBREG_REG(x), VOIDmode, fn_names);
        free(fn_names);
        return;
    }
    ...
    if (GET_CODE(x) == PARALLEL) {
        for (i = XVECLEN(x, 0) - 1; i >= 0; --i)
            invalidate(XVECEXP(x, 0, i), VOIDmode, fn_names);
        free(fn_names);
        return;
    }
    ...
    if (GET_CODE(x) == EXPR_LIST) {
        invalidate(XEXP(x, 0), VOIDmode, fn_names);
        free(fn_names);
        return;
    }
    ...
    for (i = 0; i < NBUCKETS; i++) {
        ...
        for (p = table[i]; p; p = next) {
            ...
            if (p->in_memory
                ...
                remove_from_table(p, i, fn_names);
        }
    }
    free(fn_names);
}
    ...
    for (i = 0; i < NBUCKETS; i++)
        for (p = table[i]; p; p = next) {
            ...
            if (GET_CODE(p->exp) != REG
                ...
                remove_from_table(p, i, "remove_invalid_refs");
        }
}
        ...
        for (p = table[i]; p; p = next) {
            ...
            if (GET_CODE(p->exp) != REG
                ...
                || (((SUBREG_WORD(exp)
                    ...
                    && SUBREG_WORD(exp) <= end))
                && refers_to_regno_p(regno, regno + 1, p->exp, NULL))
                remove_from_table(p, i, "remove_invalid_subreg_refs");
        }
}
...
rtx x;
{
    ...
    if (static_file) {
        fprintf(static_file, "Attempting to rehash entries mentioning:\n");
        print_rtl(static_file, x);
        fprintf(static_file, "\n");
    }
    for (i = 0; i < NBUCKETS; i++)
        for (p = table[i]; p; p = next) {
            ...
            if (GET_CODE(p->exp) != REG && reg_mentioned_p(x, p->exp)
                ...
                if (p->prev_same_hash)
                    ...
                    table[i] = p->next_same_hash; // table_write
                }
                ...
                table[hash] = p; // table_write
                if (static_file) {
                    fprintf(static_file, "(unk move operation: old hash: %d, new hash: %d)", i, hash);
                    print_rtl(static_file, p->exp);
                    fprintf(static_file, "\n");
                }
            }
        }
    ...
    if (in_table)
        for (hash = 0; hash < NBUCKETS; hash++)
            for (p = table[hash]; p; p = next) {
                ...
                if (p->in_memory) {
                    remove_from_table(p, hash, "invalidate_for_call");
                    continue;
                }
    ...
    if (GET_CODE(insn) == CALL_INSN) {
        for (tem = CALL_INSN_FUNCTION_USAGE(insn); tem; tem = XEXP(tem, 1))
            if (GET_CODE(XEXP(tem, 0)) == CLOBBER)
                invalidate(SET_DEST(XEXP(tem, 0)), VOIDmode, __func__);
    }
        ...
        for (i = 0; i < lim; i++) {
            ...
            if (GET_CODE(y) == CLOBBER) {
                ...
                if (GET_CODE(clobbered) == REG
                    ...
                    invalidate(clobbered, VOIDmode, __func__);
                else if (GET_CODE(clobbered) == STRICT_LOW_PART
                     || GET_CODE(clobbered) == ZERO_EXTRACT)
                    invalidate(XEXP(clobbered, 0), GET_MODE(clobbered), __func__);
            }
        }
        ...
        for (i = 0; i < lim; i++) {
            ...
            if (GET_CODE(y) == SET) {
                ...
                if (GET_CODE(SET_SRC(y)) == CALL) {
                    ...
                    invalidate(SET_DEST(y), VOIDmode, __func__);
        ...
        /* If destination is volatile, invalidate it and then do no further
           processing for this assignment.  */
            ...
            if (GET_CODE(dest) == REG || GET_CODE(dest) == SUBREG
                || GET_CODE(dest) == MEM)
                invalidate(dest, VOIDmode, __func__);
            else if (GET_CODE(dest) == STRICT_LOW_PART
                 || GET_CODE(dest) == ZERO_EXTRACT)
                invalidate(XEXP(dest, 0), GET_MODE(dest), __func__);
            sets[i].rtl = 0;
        }
...
invalidate_memory()
{
    ...
    for (i = 0; i < NBUCKETS; i++)
        for (p = table[i]; p; p = next) {
            next = p->next_same_hash;
            if (p->in_memory)
                remove_from_table(p, i, "invalidate_memory");
        }
}
...
register rtx addr;
{
    ...
    if ((GET_CODE(addr) == PRE_DEC || GET_CODE(addr) == PRE_INC
        ...
        if (TEST_HARD_REG_BIT(hard_regs_in_table, STACK_POINTER_REGNUM))
            invalidate(stack_pointer_rtx, VOIDmode, __func__);
        return 1;
    }
    return 0;
}
    ...
    if (GET_CODE(x) == CLOBBER) {
        ...
        if (ref) {
            if (GET_CODE(ref) == REG || GET_CODE(ref) == SUBREG
                || GET_CODE(ref) == MEM)
                invalidate(ref, VOIDmode, __func__);
            else if (GET_CODE(ref) == STRICT_LOW_PART
                 || GET_CODE(ref) == ZERO_EXTRACT)
                invalidate(XEXP(ref, 0), GET_MODE(ref), __func__);
        }
    }else if (GET_CODE(x) == PARALLEL) {
        register int i;
        for (i = XVECLEN(x, 0) - 1; i >= 0; i--) {
    ...
    if (last_jump_equiv_class)
        for (p = last_jump_equiv_class->first_same_value; p;
            ...
            if (GET_CODE(p->exp) == MEM || GET_CODE(p->exp) == REG
                ...
                invalidate(p->exp, VOIDmode, __func__);
            else if (GET_CODE(p->exp) == STRICT_LOW_PART
                 || GET_CODE(p->exp) == ZERO_EXTRACT)
                invalidate(XEXP(p->exp, 0), GET_MODE(p->exp), __func__);
        }
...
int around_loop;
{
    ...
    for (insn = from; insn != to; insn = NEXT_INSN(insn)) {
        ...
        if (code != NOTE && num_insns++ > 1000) {
            if (static_file) {
                fprintf(static_file, "Performing hash table flush.\n");
            }
            for (i = 0; i < NBUCKETS; i++)
                for (p = table[i]; p; p = table[i]) {
                    ...
                    if (GET_CODE(p->exp) == REG)
                        invalidate(p->exp, p->mode, "hash table flush");
                    else
                        remove_from_table(p, i, "hash table flush");
                }
